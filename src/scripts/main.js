...
// /filter() filters based on boolean criteria; returns array
...
const populateNameAddress = (businesses) => {
    document.querySelector("#output").innerHTML = "<h1>Active Businesses</h1>"
    businesses.forEach(business => {
            ...
            newArticle.innerHTML =`
                ...
                <hr/>
            `
            document.querySelector("#output").appendChild(newArticle)
        
    })
}


// populateNameAddress(mfgBusinesses)


// .map() loops through and puts the results into an array
...
// const populateAgentInfo = (agentArray) => {
//     document.querySelector("#output").innerHTML = "<h1>Active Agents</h1>"
...
//             /* WHEN  USER PRESSES ENTER, FIND MATCHING BUSINESS case insensitive - only first item in the array*/
//             const foundAgent = businesses.find(
//                 // find returns object that first evaluates true
//                 business =>
...
const totalSpend = businesses.forEach(business => {

    let total = parseFloat(business.orders.reduce((total, currentSpend) => total += currentSpend).toFixed(2))

    business.orderTotal = total
    console.log(total)

})

const bigSpenders = businesses.filter(business => {
    let bigSpenderFilter = false
    business.orders.forEach( order => {
        if (order > 9000) {
        bigSpenderFilter = true
        }
    })
    return bigSpenderFilter
})

populateNameAddress(bigSpenders)
